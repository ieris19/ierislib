    ...
    class MoneyTest {
        ...
        class SetterTest {
            ...
            class EdgeValueTests {
                ...
                void minLimit() {
                    assertDoesNotThrow(() -> account.setAmount(Money.MAX_MONEY));
                }

                @Test
                @DisplayName("Min value")
                void maxLimit() {
                    assertDoesNotThrow(() -> account.setAmount(Money.MIN_MONEY));
                }
            }
            ...
            class NormalValueTest {
                ...
                void positiveInRange() {
                    assertDoesNotThrow(() -> account.setAmount(new Random().nextDouble(0, Money.MAX_MONEY)));
                }

                @Test
                @DisplayName("Negative Integer within range")
                void negativeInRange() {
                    assertDoesNotThrow(() -> account.setAmount(new Random().nextDouble(Money.MIN_MONEY, 0)));
                }
            }
            ...
            class RoundingValueTest {
                ...
                @Test
                @DisplayName("Rounds down under 0.005")
                void roundDown() {
                    trueValue = 100.2545;
                ...
                @Test
                @DisplayName("Rounds up at exactly 0.005")
                void roundUpEquals() {
                    trueValue = (-100.255);
                    account.setAmount(trueValue);
                ...
                @Test
                @DisplayName("Rounds up above 0.005")
                void roundUp() {
                    trueValue = (100.2599);
                    account.setAmount(trueValue);
                    assertEquals(100.26, account.getAmount());
        ...
        class AdditionTest {
            ...
            class SmallAdditionsTest {
                @Test
                @DisplayName("Negligible amount rounded down")
                void addVerySmall() {
                ...
                @Test
                @DisplayName("Negligible amount rounded up")
                void addSmall() {
            ...
            @Test
            @DisplayName("Correctly adds expected amounts")
            void simpleAdditions() {
                account.addMoney(5);
            ...
            class BigAdditionsTest {
                @Test
                @DisplayName("Adding exactly to max amount")
                void exactMax() {
                    account.setAmount(999999995);
                    account.addMoney(5);
                    assertEquals(Money.MAX_MONEY, account.getAmount());
                }

                @Test
                @DisplayName("Adding way over the max")
                void bigMax() {
                    account.setAmount(Money.MAX_MONEY);
                    account.addMoney(Long.MAX_VALUE + Money.MAX_MONEY);
                    assertEquals(Money.MAX_MONEY, account.getAmount());
                }
            }
        }
        ...
        class SubtractionTest {
            ...
            class SubtractionLogicTest {
                @Test
                @DisplayName("Not owing returns correct value")
                void minusNotOwe() {
                ...
                @Test
                @DisplayName("Owing returns the correct value")
                void minusOwe() {
            ...
            @Test
            @DisplayName("Doesn't allow for overflow")
            void subtractionOverflowTest() {
                account.subtractMoney(Long.MAX_VALUE, true);
                assertEquals(Money.MIN_MONEY, account.getAmount());
            }
        ...
        class TransferTest {
            ...
            class ExceptionInheritanceTest {
                @Test
                @DisplayName("Transfer too little money")
                void lowAmountException() {
                    assertThrows(IllegalArgumentException.class, () -> Money.transfer(wallet, account, 0.001));
                }

                @Test
                @DisplayName("Insufficient funds")
                void insufficientException() {
                    assertThrows(IllegalArgumentException.class, () -> Money.transfer(wallet, account, INITIAL_VALUE + 1));
                }

                @Test
                @DisplayName("Transfer of negative funds")
                void negativeValueException() {
                    assertThrows(IllegalArgumentException.class, () -> Money.transfer(wallet, account, -500));
                }
            }

            @Test
            @DisplayName("Transfer math is correct")
            void correctTransfer() {
                Money.transfer(wallet, account, 5.2558);
                assertEquals(INITIAL_VALUE - 5.26, wallet.getAmount());
                assertEquals(INITIAL_VALUE + 5.26, account.getAmount());
                Money.transfer(account, wallet, 5.26);
